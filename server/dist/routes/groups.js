...
router.post('/:groupId/invitations', async (req, res, next) => {
    try {
        const { groupId } = req.params;
        const userId = req.user.id;
        ...
        const [existingInvite] = await db
            .select()
            .from(groupInvitations)
            .where(and(eq(groupInvitations.groupId, groupId), eq(groupInvitations.invitedUserId, targetUserId), eq(groupInvitations.status, 'pending')))
            .limit(1);
        if (existingInvite) {
...
router.post('/:groupId/invite-friend/:friendId', async (req, res, next) => {
    try {
        ...
        const [existingInvite] = await db
            .select()
            .from(groupInvitations)
            .where(and(eq(groupInvitations.groupId, groupId), eq(groupInvitations.invitedUserId, friendId), eq(groupInvitations.status, 'pending')))
            .limit(1);
        if (existingInvite) {
...
router.post('/invitations/:invitationId/accept', async (req, res, next) => {
    try {
        ...
        const [invitation] = await db
            .select()
            .from(groupInvitations)
            .where(and(eq(groupInvitations.id, invitationId), eq(groupInvitations.invitedUserId, userId), eq(groupInvitations.status, 'pending')))
            .limit(1);
        if (!invitation) {
...
// Cancel invitation
router.delete('/:groupId/invitations/:invitationId', async (req, res, next) => {
    try {
